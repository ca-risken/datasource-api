...
func TestDeleteCodeScanRepository(t *testing.T) {
	type args struct {
		ProjectID           uint32
		CodeGitHubSettingID uint32
		RepositoryFullName  string
	...
	cases := []struct {
	...
	}{
		{
			...
			mockClosure: func(mock sqlmock.Sqlmock) {
				mock.ExpectExec(
					regexp.QuoteMeta(deleteCodeScanRepository)).
					WillReturnResult(sqlmock.NewResult(1, 1))
			},
		},
		{
			name:    "NG wrong project_id - should not delete (JOIN mismatch)",
			args:    args{ProjectID: 999, CodeGitHubSettingID: 1, RepositoryFullName: "owner/repo"},
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				// No rows deleted because JOIN condition doesn't match (different project_id)
				mock.ExpectExec(
					regexp.QuoteMeta(deleteCodeScanRepository)).
		...
		{
			name:    "NG wrong github_setting_id - should not delete",
			args:    args{ProjectID: 1, CodeGitHubSettingID: 999, RepositoryFullName: "owner/repo"},
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				// No rows deleted because condition doesn't match (different github_setting_id)
				mock.ExpectExec(
					regexp.QuoteMeta(deleteCodeScanRepository)).
					WillReturnResult(sqlmock.NewResult(0, 0))
			},
		...
		{
			name:    "NG wrong repository_full_name - should not delete",
			args:    args{ProjectID: 1, CodeGitHubSettingID: 1, RepositoryFullName: "wrong/repo"},
			wantErr: false,
			mockClosure: func(mock sqlmock.Sqlmock) {
				// No rows deleted because condition doesn't match (different repository_full_name)
				mock.ExpectExec(
					regexp.QuoteMeta(deleteCodeScanRepository)).
					WillReturnResult(sqlmock.NewResult(0, 0))
			},
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			c.mockClosure(mock)
			err = db.DeleteCodeScanRepository(ctx, c.args.ProjectID, c.args.CodeGitHubSettingID, c.args.RepositoryFullName)
			if err != nil && !c.wantErr {
				t.Fatalf("Unexpected error: %+v", err)
			}
			if err := mock.ExpectationsWereMet(); err != nil {
