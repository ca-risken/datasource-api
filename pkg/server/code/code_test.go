...
func TestVerifyGitHubAppInstallation(t *testing.T) {
	now := time.Now()
	installationID := uint64(12345)
	cases := []struct {
		name                   string
		...
		{
			name:              "NG invalid param",
			input:             &code.VerifyGitHubAppInstallationRequest{ProjectId: 1},
			wantErr:           true,
			githubClientError: errors.New("should not be called"),
		},
		{
			name:  "NG not github app auth mode",
			input: &code.VerifyGitHubAppInstallationRequest{ProjectId: 1, GithubSettingId: 1},
			mockGitHubSetting: &model.CodeGitHubSetting{
				CodeGitHubSettingID: 1, ProjectID: 1, AuthMode: code.GitHubAuthModePersonalAccessToken,
			},
			wantErr: true,
		},
		{
			name:  "NG verification failed",
			input: &code.VerifyGitHubAppInstallationRequest{ProjectId: 1, GithubSettingId: 1},
			mockGitHubSetting: &model.CodeGitHubSetting{
...
func TestGetGitHubAppInstallationStatus(t *testing.T) {
	cases := []struct {
		...
		{
			name:    "NG invalid param",
			input:   &code.GetGitHubAppInstallationStatusRequest{ProjectId: 1},
			wantErr: true,
		},
		{
...
func TestVerifyGitHubAppUser(t *testing.T) {
	...
	cases := []struct {
		...
		{
			name:              "NG invalid param",
			input:             &code.VerifyGitHubAppUserRequest{ProjectId: 1, GithubSettingId: 1},
			wantErr:           true,
			githubClientError: errors.New("should not be called"),
		},
		{
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			got, err := svc.VerifyGitHubAppUser(context.Background(), c.input)
			if !c.wantErr && err != nil {
				t.Fatalf("Unexpected error occured: %+v", err)
			}
			if c.wantErr && err == nil {
...
func TestInvokeScanDependency(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if client == nil {
				client = newFakeGithubClient([]*ghub.Repository{
					{FullName: ghub.String("owner/repo"), ID: ghub.Int64(1)},
				}, nil)
			}
			block, _ := aes.NewCipher([]byte("1234567890123456"))
			svc := CodeService{repository: mockDB, sqs: c.mockQueue, logger: logging.NewLogger(), githubClient: client, cipherBlock: block}
			if c.mockGetDependencyResponse != nil || c.mockGetDependencyError != nil {
				...
				if c.mockGetDependencyError == nil && c.mockGetDependencyResponse != nil {
					// InvokeScanDependency now calls GetDependencySetting twice (initial + listDependencyTargetRepository)
					callCount = 2
				}
				mockDB.On("GetDependencySetting", test.RepeatMockAnything(3)...).Return(c.mockGetDependencyResponse, c.mockGetDependencyError).Times(callCount)
			}
			if c.mockGetGitHubSettingResponse != nil || c.mockGetGitHubSettingError != nil {
				// In InvokeScanDependency, GetGitHubSetting is called once when GetDependencySetting succeeds.
				callCount := 0
				if c.mockGetDependencyError == nil && c.mockGetDependencyResponse != nil {
					callCount = 1
				}
	...
	for _, c := range cases {
		t.Run(c.name, func(t *testing.T) {
			...
			if c.mockGetDependencyResponse != nil || c.mockGetDependencyError != nil {
				callCount := 1
				if c.mockGetDependencyError == nil && c.mockGetDependencyResponse != nil {
					// InvokeScanDependency (called from InvokeScanAll) calls GetDependencySetting twice
					callCount = 2
				}
				mockDB.On("GetDependencySetting", test.RepeatMockAnything(3)...).Return(c.mockGetDependencyResponse, c.mockGetDependencyError).Times(callCount)
			}
...
type FakeGithubClient struct {
	...
	installationStatus *code.GitHubAppInstallationStatus
