...
func newCloudTrailClient(defaultRegion string, logger logging.Logger) cloudTrailAPI {
...
func (c *cloudTrailClient) lookupEvents(ctx context.Context, req *activity.ListCloudTrailRequest, role, externalID string) (*activity.ListCloudTrailResponse, error) {
	...
	input, err := generateLookupEventInput(ctx, req)
	if err != nil {
		return nil, err
	}
...
func generateLookupEventInput(ctx context.Context, req *activity.ListCloudTrailRequest) (*cloudtrail.LookupEventsInput, error) {
	...
	if !zero.IsZeroVal(req.NextToken) {
		nextToken, err := decodeBase64(ctx, req.NextToken)
		if err != nil {
			return nil, err
		}
		lookupInput.NextToken = aws.String(nextToken)
	}
