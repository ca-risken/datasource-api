TARGETS = aws google diagnosis osint code datasource azure remediation
...
proto-validate: fmt
	for svc in "datasource" "azure" "remediation"; do \
		protoc \
			--proto_path=proto \
			--error_format=gcc \
...
.PHONY: lint
lint: FAKE
	GO111MODULE=on GOFLAGS=-buildvcs=false golangci-lint run --timeout $${GOLANGCI_LINT_TIMEOUT:-15m}
